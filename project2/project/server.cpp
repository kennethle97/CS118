...
Server::Server(std::string config) {
    parse_config(config);
    const char* packet = "\x45\x00\x00\x3C\x1E\xDC\x40\x00\x40\x06\x00\x00\xC0\xA8\x01\x02\xAC\x10\x00\x0A";
...
Server::TCP_Header Server::parse_TCP_Header(uint8_t ipv4_header_length, const char* packet) {
    TCP_Header tcp_header;
    const uint8_t* buffer = reinterpret_cast<const uint8_t*>(packet);
    // Calculate the offset for the TCP header based on the header length
    int offset = ipv4_header_length * 4;
    const uint8_t* tcp_header_buffer = buffer + offset;
    memcpy(&tcp_header, tcp_header_buffer, sizeof(TCP_Header));

    // Create a TCP_Header struct and populate its fields from the buffer

    tcp_header.source_port = (tcp_header_buffer[0] << 8) | tcp_header_buffer[1];
    tcp_header.destination_port = (tcp_header_buffer[2] << 8) | tcp_header_buffer[3];
    tcp_header.sequence_number = (tcp_header_buffer[4] << 24) | (tcp_header_buffer[5] << 16) | (tcp_header_buffer[6] << 8) | tcp_header_buffer[7];
    tcp_header.acknowledgment_number = (tcp_header_buffer[8] << 24) | (tcp_header_buffer[9] << 16) | (tcp_header_buffer[10] << 8) | tcp_header_buffer[11];
    tcp_header.data_offset_and_flags = (tcp_header_buffer[12] << 8) | tcp_header_buffer[13];
    tcp_header.flags = tcp_header_buffer[13] & 0x3F; // Extract flags by masking upper 2 bits
    tcp_header.window_size = (tcp_header_buffer[14] << 8) | tcp_header_buffer[15];
    tcp_header.checksum = (tcp_header_buffer[16] << 8) | tcp_header_buffer[17];
    tcp_header.urgent_pointer = (tcp_header_buffer[18] << 8) | tcp_header_buffer[19];
...
void Server::printIPv4Header(const IPv4_Header& header) {
    ...
    std::cout << "Header Length: " << static_cast<int>(header.header_length) << '\n';
    std::cout << "Type of Service: " << static_cast<int>(header.type_of_service) << '\n';
    ...
    std::cout << "Time to Live: " << static_cast<int>(header.time_to_live) << '\n';
